...
list_skenario = range(2,13) #diganti dengan list
...
for skenario in list_skenario:
    ...
    for i in range(n):
        ...
        #5 teratas, bobot dan frekuensi
            ...
            if skenario == 4: #frekuensi * bobot normal
                kandidat = sorted(kandidat, reverse = True, key = lambda x: x[0]*x[1])
            elif skenario == 5: #frekuensi * bobot normalisasi(log 10)
                kandidat = [[i[0],1 + math.log(i[1],10),i[2]] for i in kandidat]
                kandidat = sorted(kandidat, reverse = True, key = lambda x: x[0]*x[1])
            elif skenario == 6: #frekuensi * bobot normalisasi(log 2)
                kandidat = [[i[0],1 + math.log(i[1],2),i[2]] for i in kandidat]
                kandidat = sorted(kandidat, reverse = True, key = lambda x: x[0]*x[1])
            elif skenario == 7: #frekuensi * bobot normalisasi(augmented 0.5)
        ...
        #5 teratas, reduksi frekuensi dan bobot, frekuensi > 1
            ...
            if skenario == 9: #frekuensi * bobot normal
                kandidat = sorted(kandidat, reverse = True, key = lambda x: (x[0]*x[1],len(x[2])))
            elif skenario == 10: #frekuensi * bobot normalisasi(log 10)
                kandidat = [[i[0],1 + math.log(i[1],10),i[2]] for i in kandidat]
                kandidat = sorted(kandidat, reverse = True, key = lambda x: (x[0]*x[1],len(x[2])))
            elif skenario == 11: #frekuensi * bobot normalisasi(log 2)
                kandidat = [[i[0],1 + math.log(i[1],2),i[2]] for i in kandidat]
                kandidat = sorted(kandidat, reverse = True, key = lambda x: (x[0]*x[1],len(x[2])))
            elif skenario == 12: #frekuensi * bobot normalisasi(augmented 0.5)
                max_bobot = max(kandidat,key=lambda x:x[1])[1]
                kandidat = [[i[0],0.5 + (0.5*i[1])/max_bobot,i[2]] for i in kandidat]
                kandidat = sorted(kandidat, reverse = True, key = lambda x: (x[0]*x[1],len(x[2])))
