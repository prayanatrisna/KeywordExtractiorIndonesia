...
list_skenario = range(13,24) #diganti dengan list

for skenario in list_skenario:
    print skenario # 13 - 23
    for i in range(n):
        n_kandidat = 5
        min_freq = 1
        fol_num = str(i+1)
        ...
        #5 teratas, bobot dan frekuensi
            ...
            if skenario == 15: #frekuensi * bobot normal
                kandidat = sorted(kandidat, reverse = True, key = lambda x: x[0]*x[1])
            elif skenario == 16: #frekuensi * bobot normalisasi(log 10)
                kandidat = [[i[0],1 + math.log(i[1],10),i[2]] for i in kandidat]
                kandidat = sorted(kandidat, reverse = True, key = lambda x: x[0]*x[1])
            elif skenario == 17: #frekuensi * bobot normalisasi(log 2)
                kandidat = [[i[0],1 + math.log(i[1],2),i[2]] for i in kandidat]
                kandidat = sorted(kandidat, reverse = True, key = lambda x: x[0]*x[1])
            elif skenario == 18: #frekuensi * bobot normalisasi(augmented 0.5)
        ...
        #5 teratas, reduksi frekuensi dan bobot, frekuensi > 1
            ...
            if skenario == 20: #frekuensi * bobot normal
                kandidat = sorted(kandidat, reverse = True, key = lambda x: (x[0]*x[1],len(x[2])))
            elif skenario == 21: #frekuensi * bobot normalisasi(log 10)
                kandidat = [[i[0],1 + math.log(i[1],10),i[2]] for i in kandidat]
                kandidat = sorted(kandidat, reverse = True, key = lambda x: (x[0]*x[1],len(x[2])))
            elif skenario == 22: #frekuensi * bobot normalisasi(log 2)
                kandidat = [[i[0],1 + math.log(i[1],2),i[2]] for i in kandidat]
                kandidat = sorted(kandidat, reverse = True, key = lambda x: (x[0]*x[1],len(x[2])))
            elif skenario == 23: #frekuensi * bobot normalisasi(augmented 0.5)
                max_bobot = max(kandidat,key=lambda x:x[1])[1]
                kandidat = [[i[0],0.5 + (0.5*i[1])/max_bobot,i[2]] for i in kandidat]
                kandidat = sorted(kandidat, reverse = True, key = lambda x: (x[0]*x[1],len(x[2])))
